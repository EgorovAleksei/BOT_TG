...
async def admin_features(message: types.Message):
    # print(f"admin_private строка 40")
...
async def add_description2(message: types.Message, state: FSMContext, session: AsyncSession):
...
async def category_choice2(message: types.Message, state: FSMContext, session: AsyncSession):
    categories = await orm_get_categories(session)
    btns = {category.name: str(category.id) for category in categories}
    await message.answer("'Выберите катеорию из кнопок.'", reply_markup=get_callback_btns(btns=btns))
