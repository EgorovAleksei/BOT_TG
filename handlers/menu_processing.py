...
from kbds.inline import (get_products_btns, get_user_cart,
...
async def products(session: AsyncSession, level: int, category, page):
...
async def carts(session: AsyncSession, level, menu_name, page, user_id, product_id):
    if menu_name == 'delete':
